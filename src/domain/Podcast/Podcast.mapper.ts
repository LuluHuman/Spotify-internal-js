...
import { Episode, Show } from "./Podcast";
import SpotifyIdentifier from "../../infra/Identifier/SpotifyIdentifier";
import { APILibraryPage } from "../../infra/api_types";
import { APIEpisodeOrChapterResponseWrapper, APIPlaylistContent } from "../Playlist/Playlist.types";
import { APIPodcastWrapper, APIShow, APIShowEpisode, APIShowEpisodes } from "./Podcast.types";
...
export function mapSavedEpisodes(spotify: Spotify, playlistItems: APIPlaylistContent["data"]["playlistV2"]["content"]) {
    return playlistItems.items.filter(i => i.itemV2.data.__typename == "Episode").map((episode) => {
        ...
        if (!episode.itemV3.data) throw new Error("episode.itemV3.data is undefined")
        return new Episode(spotify, {
